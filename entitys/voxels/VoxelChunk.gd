...
func get_shape(VOXEL_DATA, mappos):
		...
		for edge in VOXEL_DATA.INTERPOLATED_EDGES:
			if edge.x == i:
				...
				if edgesaxis[edge.y] == 0:
					tempvector.x = interpolateVertsX(
						map.map[d.x + mappos.x][d.y + mappos.y],
						map.map[b.x + mappos.x][b.y + mappos.y],
						d,
						b
					)
				else:
					tempvector.y = interpolateVertsY(
						map.map[d.x + mappos.x][d.y + mappos.y],
						map.map[b.x + mappos.x][b.y + mappos.y],
						d,
						b
